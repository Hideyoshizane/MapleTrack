...
module.exports = {
	getList: async (req, res) => {
		try {
			...
			const bossListFetch = await bossList.findOne({
				userOrigin: username,
			});
	...
	increaseBoss: async (req, res) => {
		try {
			...
			const foundServer = ListFound.server.find(
				(servers) => servers.name === server,
			);
			const foundCharacter = foundServer.characters.find(
				(character) => character.class === characterClass,
			);
			const foundBoss = foundCharacter.bosses.find(
				(boss) => boss.name == bossName && boss.difficulty == difficult,
			);

			foundBoss.checked = checkMark;

			foundServer.totalGains = checkMark
				? foundServer.totalGains + Number(value)
				: foundServer.totalGains - Number(value);
			foundServer.weeklyBosses = checkMark
				? foundServer.weeklyBosses + 1
				: foundServer.weeklyBosses - 1;
	...
	saveBossChange: async (req, res) => {
		try {
			...
			const listFound = await bossList.findOne({
				userOrigin: characterList.userOrigin,
			});
			const foundServer = listFound.server.find(
				(servers) => servers.name === characterList.name,
			);

			for (const character of characterList.characters) {
				const foundCharacter = foundServer.characters.find(
					(serverCharacter) =>
						serverCharacter.class === character.class,
				);
				for (const bossData of character.bosses) {
					const existingBossIndex = foundCharacter.bosses.findIndex(
						(existingBoss) =>
							existingBoss.name == bossData.name &&
							existingBoss.reset == bossData.reset,
					);
					if (existingBossIndex !== -1) {
						const existingBoss =
							foundCharacter.bosses[existingBossIndex];
						if (bossData.reset === 'Daily') {
							existingBoss.DailyTotal = bossData.DailyTotal;
						}
						if (
							(existingBoss.reset === 'Weekly' ||
								existingBoss.reset === 'Monthly') &&
			...
			for (const character of foundServer.characters) {
				const foundCharacter = characterList.characters.find(
					(listCharacter) => listCharacter.class === character.class,
				);
				character.bosses = character.bosses.filter((onDatabaseBoss) => {
					return foundCharacter.bosses.some(
						(onRequestBoss) =>
							onDatabaseBoss.name === onRequestBoss.name &&
							onDatabaseBoss.reset === onRequestBoss.reset,
					);
				});
			}
